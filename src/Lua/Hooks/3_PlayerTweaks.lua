...
addHook("MobjDeath", function(target, inflictor, source)
	if CV_PTD.scoreonkill.value and gametype == GT_PIZZATIMEDELUXE and source and source.valid and source.player and source.player.valid then
...
addHook("PlayerCanDamage", function(player, mobj)
	if PTD.pizzatime and CV_PTD.killwhilerunning.value and player.speed >= skins[player.mo.skin].runspeed and not mobj.player then
		return true
	end
end)